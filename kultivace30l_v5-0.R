...
mu_kon <- 0.055   # minim?ln? r?stov? rychlost (na konci kultivace)
par_k <- 0.2    # prohnut? k?ivky pr?b?hu r?stov? rychlosti

OD <- 8.65      # OD inokula

par_vRPM <- 1/0.88144   # prepocet: otacky pumpy [%] = feed [ml/min] * par_vRPM

{
  ...
  opt_fct <- function(delka_kultivace){
    ...
    # zadej OD; vypocte se DWT jako OD / 1.8136 * objem inokula + odhadovan? zisk z batch
